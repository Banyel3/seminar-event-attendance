...
const NavBar = () => (
  <div className="flex gap-2 p-1 bg-white/60 backdrop-blur-sm rounded-xl mb-6 w-full max-w-xs mx-auto shadow-sm">
    <Link
      href="/"
      className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-slate-500 hover:text-emerald-600 font-semibold text-sm transition-colors"
    >
      ...
      <Card className="max-w-md w-full shadow-md animate-in fade-in slide-in-from-bottom-4 duration-500 bg-white/90 backdrop-blur-md">
        ...
        <CardContent>
          {error && (
            <Alert
              variant="destructive"
              className={`mb-6 ${notRegistered ? "bg-amber-50 border-amber-200 text-amber-800" : "bg-red-50 border-red-200 text-red-800"}`}
            >
              <AlertCircle
                className="h-4 w-4"
                color={notRegistered ? "#D97706" : "#EF4444"}
              />
              <AlertTitle>
                {notRegistered ? "Not Registered" : "Error"}
              </AlertTitle>
              <AlertDescription>
                {error}
                {notRegistered && (
